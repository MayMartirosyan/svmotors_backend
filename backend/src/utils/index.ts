...
export function toCamelCase<T>(obj: T): T {
  ...
  return Object.keys(obj).reduce((result:any, key) => {
    ...
    result[camelKey] = value instanceof Date ? new Date(value).toISOString() : toCamelCase(value);
    return result;
  }, {} as T);
}
...
export function toSnakeCase<T>(obj: T): T {
  ...
  return Object.keys(obj).reduce((result:any, key) => {
    const snakeKey = key
      ...
      .replace(/^_/, "");
    const value = obj[key as keyof T];
    result[snakeKey as keyof T] = value instanceof Date ? new Date(value).toISOString() : toSnakeCase(value);
    return result;
  }, {} as T);
}

function getRelativePath(img: string): string {
  return img.startsWith(`https://${process.env.S3_BUCKET_NAME}`)
    ? img.split(".com/")[1]
...
export async function handleMultiImageUpdate(
  ...
  const imagesToDelete = previousRelativePaths.filter(
    (prevPath) => !currentRelativePaths.includes(prevPath)
  ).map((path) => {
  ...
  if (uploadDir) {
    imagesToDelete.forEach((img) => {
      const filePath = path.join(uploadDir, getRelativePath(img).replace("/Uploads/", ""));
